 ...
 * FunctionParser
 *
 * The FunctionParser has the ability to take a reflected function or method and retrieve it's
 * code. In the case of a Closure, it will also get the names and values of any closed upon
 * variables (i.e. variables in the "use" statement). It relies on PHP lexical scanner, so the
 * PHP tokenizer must be enabled in order to use the library.
 *
...
class FunctionParser
{
	...
	public function __construct(\ReflectionFunctionAbstract $reflection)
	{
		...
		{
			throw new \InvalidArgumentException('You cannot parse the code of an internal PHP'
				. ' function.');
		}
	...
	protected function _parseCode()
	{
		...
		foreach ($this->tokenizer as $token)
		{
			 ...
			 * AFTER PARSING
			 *
			 * After the parsing is complete, we need to make sure there are no other T_FUNCTION
			 * tokens found, which would indicate a possible ambiguity in the function code we
			 * retrieved. This should only happen in situations where the code is minified or
			 * poorly formatted.
			 */
			if ($parsing_complete)
			{
			 ...
			 * WHILE PARSING
			 *
			 * Scan through the tokens (while keeping track of braces), and reconstruct the code
			 * from the parsed tokens.
			 */
